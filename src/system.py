...
#添加联系人
    def add_contact(self, name, phone_number, remark="", gender=""):
        # 检查手机号唯一性（所有联系人：正常+隐藏，不能共用同一手机号）
        all_contacts = self.contacts + self.hidden_contacts
        for c in all_contacts:
            if c.get("phone_number") == phone_number:
                print(f"添加失败：手机号 {phone_number} 已被联系人 {c.get('name')} 使用（{'' if c in self.contacts else '隐藏'}列表）。")
                return False
    ...
    def search_contact(self, name):
        """按精确姓名查找联系人，返回所有匹配的联系人字典列表（仅在正常联系人中查找）。"""
        # 修复：返回所有同名联系人，而非第一个
        matched = [c for c in self.contacts if c.get("name") == name]
    ...
    def delete_contact(self, name):
        ...
        for contact in contacts:
            ...
            try:
                ...
                print(f"删除 {contact.get('name')} 失败：无法写入 WAL。")
                continue
            ...
            try:
                if op == "add":
                    ...
                    if target:
                        ...
                        new_name = data.get("new_name")  # 修复原代码的语法错误
    ...
    def set_blacklist(self, name, blacklisted=True):
        ...
        for contact in contacts:
            ...
            try:
                ...
                print(f"操作 {contact.get('name')} 失败：无法写入 WAL。")
                continue
        ...
        state = "已加入黑名单" if blacklisted else "已移出黑名单"
        print(f"所有名为 {name} 的联系人 {state}。")
        return True

    # 统计功能
    def print_stats(self, include_hidden=False):
        """统计联系人中各个性别人数以及黑名单人数。默认只统计正常联系人，include_hidden=True 则同时统计隐藏联系人。"""
        total_list = list(self.contacts)
        if include_hidden:
            total_list += list(self.hidden_contacts)
        ...
        for c in total_list:
            # 统计性别
            g = str(c.get("gender", "")).strip().lower() or "未知"
        ...
        print("统计信息：")
        print(f"  总计联系人（包含隐藏={include_hidden}）：{len(total_list)}")
        print("  按性别统计：")
        for g, cnt in sorted(gender_counts.items(), key=lambda x: x[0]):
            print(f"    {g}：{cnt}")
        print(f"  黑名单人数（包含隐藏={include_hidden}）：{blacklisted_count}")
...
if __name__=="__main__":
    ...
    while True:
        ...
        if choice == "1":
            ...
            phone_number = input("请输入联系人电话：").strip()
            gender = input("请输入性别（可选）：").strip()
            remark = input("请输入备注（可选,不输入默认空白）：").strip()
            ...
            if mode == "1":
                ...
                contacts = cl.search_contact(name)
                if not contacts:
                    print("未找到联系人（隐藏联系人不会在正常查询中显示）。")
                else:
                    print(f"找到 {len(contacts)} 个名为 {name} 的联系人：")
                    for idx, contact in enumerate(contacts, 1):
                        print(f"{idx}. 名称: {contact.get('name')}, 电话: {contact.get('phone_number')}, 性别: {contact.get('gender')}, 备注: {contact.get('remark')}, 状态: {'黑名单' if contact.get('blacklisted') else '正常'}")
            elif mode == "2":
                prefix = input("请输入姓名前缀：").strip()
                results = cl.search_by_prefix(prefix)
                if not results:
                    print("未找到匹配的联系人。")
                else:
                    print(f"找到 {len(results)} 个匹配前缀的联系人：")
                    for c in results:
                        print(f"名称: {c.get('name')}, 电话: {c.get('phone_number')}, 备注: {c.get('remark')}")
            elif mode == "3":
                suffix = input("请输入手机号后缀（例如尾号）：").strip()
                results = cl.search_by_phone_suffix(suffix)
                if not results:
                    print("未找到匹配的联系人。")
                else:
                    print(f"找到 {len(results)} 个匹配后缀的联系人：")
                    for c in results:
                        print(f"名称: {c.get('name')}, 电话: {c.get('phone_number')}, 备注: {c.get('remark')}")
            elif mode == "4":
                ...
                if not results:
                    ...
                    print(f"找到 {len(results)} 个匹配性别的联系人：")
                    for c in results:
                        print(f"名称: {c.get('name')}, 电话: {c.get('phone_number')}, 备注: {c.get('remark')}")
            else:
                print("无效的查找方式。")
            ...
            if not contacts:
                ...
                # 若有多个同名联系人，默认选择第一个
                if len(contacts) > 1:
                    print(f"找到 {len(contacts)} 个名为 {name} 的联系人，默认修改第一个：")
                    print(f"当前信息：名称: {contacts[0].get('name')}, 电话: {contacts[0].get('phone_number')}, 性别: {contacts[0].get('gender')}, 备注: {contacts[0].get('remark')}")
                else:
                    print(f"当前信息：名称: {contacts[0].get('name')}, 电话: {contacts[0].get('phone_number')}, 性别: {contacts[0].get('gender')}, 备注: {contacts[0].get('remark')}")
                ...
                new_name = None if new_name == "" else new_name
                new_phone = None if new_phone == "" else new_phone
                new_remark = None if new_remark == "" else new_remark
                new_gender = None if new_gender == "" else new_gender
            ...
            inc = input("是否包含隐藏联系人统计？(y/n，默认n)：").strip().lower()
            include_hidden = True if inc == "y" else False
            cl.print_stats(include_hidden=include_hidden)
